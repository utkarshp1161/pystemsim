...
class SidpyDataset(Dataset):
    def __init__(self, image_dataset: sidpy.Dataset, label_dataset: sidpy.Dataset, transform=None):
        """
        Args:
            image_dataset (sidpy.Dataset): sidpy dataset of images [N, H, W]
            label_dataset (sidpy.Dataset): sidpy dataset of labels [N, C, H, W]
            transform (callable, optional): transform applied to both image and labels
        """
        self.images = image_dataset
        self.labels = label_dataset
        self.transform = transform
        self.length = image_dataset.shape[0]

        assert image_dataset.shape[0] == label_dataset.shape[0], \
            "Image and label datasets must have the same number of samples"
    ...
    def __getitem__(self, idx):
        ...
        # Normalize images (if they’re not already normalized)
        if image.max() > 1.0:
            image = image / 255.0
        if labels.max() > 1.0:
            labels = labels / 255.0
        ...
        labels = torch.tensor(labels)              # [C, H, W]
...
def get_dataloaders(images_dir, labels_dir, batch_size, val_split=0.1, test_split=0.1):
    # some work here
    # take in a dataset object, determine
