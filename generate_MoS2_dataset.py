...
# Simulation based images - first 1000
crop_size = 512
pixel_size = 0.078125 # Angstrom/pixel, determines number of points, aka resolution of maps.  the xtal determines the fov
n_crops = 10 # number of crops per large image

print('making images from gb simulation:')
while image_counter < 1000:
    ...
    airy_disk_size = 1
    shot_noise = rng.uniform(0.8, 0.94)
    ...
    for image, label_set in zip(batch_ims, batch_masks):
        if np.average(image) > 0.05: # only save images with atoms - this is a hacky way to do this
...
print('making images from pristine MoS2 w/ holes:')
xtal = read('./crystal_files/MoS2.cif') 
# Remove bottom layer atoms (bilayer cif)
com = xtal.get_center_of_mass()
atoms_to_delete = [atom.index for atom in xtal if atom.position[2] < com[2]]
del xtal[atoms_to_delete]
xtal.center()

xtal_l1 = xtal.repeat((100,100,1))
xtal_l2 = xtal_l1.copy()
combined_xtal = xtal_l1 + xtal_l2
positions = combined_xtal.get_positions()[:, :2]
xmin, xmax = np.min(positions[:, 0]), np.max(positions[:, 0])
ymin, ymax = np.min(positions[:, 1]), np.max(positions[:, 1])
...
while image_counter < n_images:
    ...
    phonon_sigma = rng.uniform(0.02, 0.1)
    rotation_l1 = rng.uniform(0, 360)
    rotation_l2 = rotation_l1 + rng.choice([-60, -30, -15, 0, 0, 0, 15, 30, 60])
